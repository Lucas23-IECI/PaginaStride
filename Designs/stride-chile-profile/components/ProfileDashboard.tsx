...
export const ProfileDashboard: React.FC<ProfileDashboardProps> = ({ onChangeView }) => {
  return (
    <div className="flex-1 h-full overflow-y-auto no-scrollbar pb-20">
      ...
      <div className="p-6 md:p-10 space-y-12">
        ...
        <section>
          <div className="flex items-end justify-between mb-6">
            ...
            <button onClick={() => onChangeView('manage-registrations')} className="text-secondary text-sm font-bold hover:underline">
              Gestionar registros
            </button>
          </div>
            ...
            <div className="min-w-[320px] snap-center group">
              <div className="bg-card-dark rounded-3xl overflow-hidden border border-white/5 h-full flex flex-col shadow-xl">
                ...
                <div className="bg-primary p-5 relative flex-grow">
                  ...
                  <button onClick={() => onChangeView('manage-registrations')} className="w-full py-2 bg-white text-primary rounded-full font-bold text-xs hover:bg-gray-100 transition-all relative z-10">
                    Ver detalles
                  </button>
                </div>
              </div>
            ...
            <div className="min-w-[320px] snap-center group">
              <div className="bg-card-dark rounded-3xl overflow-hidden border border-white/5 h-full flex flex-col shadow-xl">
                ...
                <div className="bg-secondary p-5 relative flex-grow">
                  ...
                  <button onClick={() => onChangeView('manage-registrations')} className="w-full py-2 bg-white text-secondary rounded-full font-bold text-xs hover:bg-gray-100 transition-all relative z-10">
                    Ver detalles
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex gap-4 text-xs font-bold text-white/40">
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-primary"></div> Community
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-2 h-2 rounded-full bg-secondary"></div> Personal
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-7 gap-2">
                {['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'].map((day) => (
                  <div key={day} className="text-center text-[10px] font-black text-white/20 uppercase tracking-widest pb-4">
                    {day}
                  </div>
                ))}
                ...
                {Array.from({length: 31}, (_, i) => i + 1).map(d => {
                    ...
                    if(d === 2 || d === 13) className += " has-event !bg-secondary/10 !text-secondary !border-secondary/30";
                    else if(d === 7 || d === 17 || d === 24) className += " has-event";
                    else if(d === 10) className += " today";
                    
                    return <div key={`curr-${d}`} className={className}>{d}</div>
                })}
              </div>
            </div>
          </div>
              ...
              <div className="flex flex-col h-full">
                <div className="relative w-full aspect-square mb-6 rounded-2xl overflow-hidden bg-black/40 border border-white/5 flex items-center justify-center">
                  <div className="text-center transform group-hover:scale-110 transition-transform duration-500">
                    ...
                    <p className="text-[8px] text-white/20 uppercase tracking-widest mt-1 italic">
                      Technical T-Shirt
                    </p>
                  </div>
                </div>
                ...
                <button className="mt-auto w-full py-3 bg-white text-black rounded-full font-bold text-sm hover:bg-gray-200 transition-all flex items-center justify-center gap-2">
