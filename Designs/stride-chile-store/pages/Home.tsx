  ...
  return (
    <div className="relative pt-20">
        ...
        <section className="relative z-10 px-6 pb-24 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {displayProducts.map((product) => (
                    <div key={product.id} className="group bg-card-dark rounded-DEFAULT overflow-hidden hover:shadow-2xl hover:shadow-primary/10 transition-all duration-300 border border-white/5 flex flex-col">
                        <Link to={`/product/${product.id}`} className="relative aspect-[4/5] overflow-hidden bg-gray-800">
                            <img 
                                alt={product.name}
                        ...
                        <div className="p-5 flex flex-col flex-grow">
                            ...
                            <div className="flex items-end justify-between mt-4">
                                <span className="text-xl font-bold text-price-gold">${product.price.toLocaleString('es-CL')}</span>
                            </div>
                            <button 
                                onClick={() => addToCart(product)}
                                className="w-full mt-4 py-3 bg-surface-dark hover:bg-white hover:text-black text-white text-xs uppercase tracking-wide font-bold rounded-full transition-colors flex items-center justify-center gap-2 group-hover:bg-gradient-primary group-hover:text-white"
                            >
        ...
        <section className="max-w-7xl mx-auto px-6 pb-24">
            <div className="relative rounded-DEFAULT overflow-hidden bg-surface-dark border border-white/5">
